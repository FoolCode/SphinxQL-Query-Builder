...
class SphinxQLTest extends PHPUnit_Framework_TestCase
{
	...
	{
		$this->sq->setVariable('AUTOCOMMIT', 0);
		$vars = $this->sq->variables();
		$this->assertEquals(0, $vars['autocommit']);

		$this->sq->setVariable('AUTOCOMMIT', 1);
		$vars = $this->sq->variables();
		$this->assertEquals(1, $vars['autocommit']);

		$this->sq->setVariable('@foo', 1, true);
		$this->sq->setVariable('@foo', array(0), true);
	}

	public function testTransactions()
	{
		$this->sq->transactionBegin();
		$this->sq->transactionRollback();
		$this->sq->transactionBegin();
		$this->sq->transactionCommit();
	}
	...
	{
		$result = $this->sq->select(SphinxQL::expr('@count'))
			->from('rt')
			->groupBy('gid')
			->execute();
